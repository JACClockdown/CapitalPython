...
class FirmamexServices:
    ...
    def hashAndPost(self, jsonParams, path):
        ...
        json_string = json.dumps(jsonParams, separators=(',', ':'), ensure_ascii=False)
    ...
    def createDocumentSet(self, name):
        params = {
            "name": name
        }
        return json.loads(self.hashAndPost(params, '/api/documentset'))

    def closeDocumentSet(self, documentSet, workflow):
        params = {
            "documentSet": documentSet,
            "workflow": workflow
        }
        return self.hashAndPost(params, '/api/documentset/close')

    def getDocumentSet(self, documentSet):
        return json.loads(self.hashAndGet('/api/documentset/' + documentSet))
    
    def workflow(self, firmamexId):
        params = {
            'ordered': True,
            'cancel': True
        }
        return self.hashAndPost(params,'/api/' + firmamexId + '/workflow')
